...
def home():
    return render_template('ehome.html')      #just added
...
def profile():
	if 'tag' in session:
		...
		return render_template('eprofile.html',username=p['name'],points=p['points'])		#just added
...
def contact():
    if request.method == 'POST':
        ...
        yag = yagmail.SMTP('endsemproject24@gmail.com', 'sainasimran1')	#to initialize
...
def register():
    if request.method == 'POST' :
        ...
        m1=mongo.db.gamer.find_one({'title': "forc"})
        if m1:
            n=m1['count']
        n1=int(n)
        nx=str(n1+1)
        ...
        #in=mongo.db.gamerz.find().sort({age:-1}).limit(1)
        #m1=mongo.db.gamer.find().sort( 'index' , -1 )


        return render_template('gametagdisp.html', id = nx)
        #return('user registered with gamertag %s' %nx)
